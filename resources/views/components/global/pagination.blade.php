...
@if ($paginator->hasPages())
    <div {{ $attributes->merge(['class' => 'px-6 py-4 border-t border-gray-200 dark:border-gray-700']) }}>
        <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
            <!-- Info -->
            <div class="text-sm text-center sm:text-left text-gray-600 dark:text-gray-400">
                Menampilkan
            ...
            <div class="flex flex-wrap justify-center items-center gap-2">
                ...
                @foreach ($paginator->getUrlRange(max(1, $paginator->currentPage() - $onEachSide), min($paginator->lastPage(), $paginator->currentPage() + $onEachSide)) as $page => $url)
                    @if ($page == $paginator->currentPage())
                        <button
                            class="hidden sm:block px-4 py-2 text-sm rounded-lg font-semibold
                                  bg-blue-600 text-white shadow-sm">
                            {{ $page }}
                        </button>
                    @else
                        <button wire:click="gotoPage({{ $page }})"
                            class="hidden sm:block px-4 py-2 text-sm rounded-lg border border-gray-300 dark:border-gray-600 
                                   bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300
                                   hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                            {{ $page }}
                        </button>
